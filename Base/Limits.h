...
static inline void Limits_Dummy_Twos_Complement_Validator___() {
	KWLKIT_COMPILE_ASSERT( (Int)((SByte)-128) == -128 );	// need 2's complement integers!
}
...
template<> struct Limits<Float>
{
	...
	static inline const Float EpsilonDenormal() {
		return 1.0e-37f;
	}
...
{
	// almost zero (above denormal)
	static const Double EpsilonDenormal() {
		return 1.0e-307;
	}
	// smallest epsilon such that 1 + eps != 1
	static inline const Double Epsilon() {
